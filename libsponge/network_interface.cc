...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    if (iter == _arp_table.end()) {
        const auto &waiting_iter = _waiting_set.find(next_hop_ip);

        if (waiting_iter == _waiting_set.end()) {
            ...
            _waiting_set[next_hop_ip] = _arp_response_default_ttl;
        }
        _waiting_arp_internet_datagrams.push_back({next_hop, dgram});
    } else {
        EthernetFrame eth_frame;
        eth_frame.header() = {iter->second.eth_addr, _ethernet_address, EthernetHeader::TYPE_IPv4};
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    ...
    if (frame.header().type == EthernetHeader::TYPE_IPv4) {
        ...
        if (is_valid_arp_request || is_valid_arp_response) {
            ...
            _waiting_set.erase(src_ip_addr);
        }
    }
    return nullopt;
    DUMMY_CODE(frame);
...
void NetworkInterface::tick(const size_t ms_since_last_tick) {
    ...
    for (auto iter = _waiting_set.begin(); iter != _waiting_set.end();) {
